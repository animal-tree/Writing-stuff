...
>
> In case there is question as to why $v' = \frac{vt'}{t}$, this is derived from the fact that velocity is distance over time and the measuring units of $K'$ are scaled as follows: $v' = \frac{v}{\frac{t}{t'}}$. If time is slower, meaning $\frac{t'}{t}$ increases, then the original velocity should be measured faster. If time is faster, meaning $\frac{t'}{t}$ decreases, then the original velocity should be measured slower.
