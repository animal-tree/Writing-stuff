...
>
> In case there is question as to why $v' = \frac{vt'}{t}$, first the intuition: "If time is slower, meaning $\frac{t'}{t}$ increases, then the original velocity should be measured faster; if time is faster, meaning $\frac{t'}{t}$ decreases, then the original velocity should be measured slower." Now the derivation: $x = vt'$ comes from the Galilean assumption w.r.t. distance $\rightarrow$ $v' = v = \frac{x}{t} = \frac{vt'}{t}$.
