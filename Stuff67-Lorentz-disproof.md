...
> - Since the below disproof considers motion only in the x-axis, an argument could be made about the two light clocks being adjacent rather than fixed in the same position; however, the two hoops (the initiating one at the start and the relay station hoop at the end) can account for this by respectively shooting photons (or electrons) from adjacent halves of the inner edge.
