...
A little more precise: the effect between the intermediary particles would be non-zero. However, their effect on the first body, without the cross-derivative interaction with a second body, would be zero. Thus, they propagate through collisions with each other without affecting teh first body, until they affect a second body. THEN their spin value makes them affect each other in the reverse direction in such a way that affects the first body, and vice versa, likewise affecting the second body gravitationally.
